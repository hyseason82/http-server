...
### vcam.c — V4L2 虚拟摄像头（RK3568车载座舱预研）

为 **基于RK3568的车载多媒体座舱** 项目做的前期原型验证，在无硬件环境下完整走通 V4L2 驱动栈。

- `v4l2_device` + `video_device`：注册 `/dev/video0`，FFmpeg/GStreamer/v4l2-ctl 直接可用
...
vcam.c                 V4L2虚拟摄像头（videobuf2 + kthread + YUYV彩条，RK3568车载座舱预研）
